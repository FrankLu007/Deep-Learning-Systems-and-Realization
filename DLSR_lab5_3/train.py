...
from model import SpecialModel
...
if __name__ == '__main__' :
	...
	validationloader = torch.utils.data.DataLoader(validationset, batch_size = args['batch_size'], num_workers = 12)
	testloader = torch.utils.data.DataLoader(testset, batch_size = args['batch_size'], num_workers = 12)
	...
	model.eval()
	with torch.no_grad() :
		lastaccuracy = forward('Validation', testloader, model)
	...
	for epoch in range(args['epoch']):  # loop over the dataset multiple times
		...
		model.eval()
		with torch.no_grad() :
			accuracy = forward('Validation', testloader, model)
